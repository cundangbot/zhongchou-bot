...
from app.services.payment_flow import confirm_seed_payment_message
...
async def _open_admin_dialog_for_user(
    bot: Bot,
    *,
    admin_id: int,
    user_id: int,
    ...
    order_id: int | None = None,
    ref_id: int = 0,
) -> tuple[ContactTicket, str]:
    """Open/reopen a support ticket and send a routable card to the support admin's private chat."""
    support_admin_id = _support_private_admin_id()
    if not support_admin_id:
        raise RuntimeError('请先配置 SUPPORT_ADMIN_ID')
    if int(admin_id) != int(support_admin_id):
        raise PermissionError('这个操作只给 SUPPORT_ADMIN_ID 对应管理员使用')
    ...
    return ticket, user_label


def _parse_admin_user_message_args(args: str | None) -> tuple[int | None, str]:
    raw = (args or '').strip()
    if not raw:
        return None, ''
    ...
    return int(token), parts[1].strip() if len(parts) > 1 else ''
...
def _beijing_day_bounds_utc_naive(day_offset: int = 0) -> tuple[datetime, datetime, str]:
    """Return one Beijing calendar day's UTC-naive bounds and display label."""
    safe_offset = max(0, int(day_offset or 0))
    start_bj = datetime.now(BEIJING_TZ).replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=safe_offset)
...
async def _handle_join_deeplink(message: Message, project_id: int) -> None:
    async with SessionLocal() as session:
        ...
        if state_value(project.status) == 'approved_wait_creator':
            await message.answer('⏳ 车主正在完成双车位支付，自动核验成功后就可以上车啦～', reply_markup=main_menu())
            return
...
async def pending_orders_paged(call: CallbackQuery):
    ...
    await _edit_panel(call,
        msg.pending_orders_list(page=page + 1, pages=max(1, (len(orders) + 4) // 5), total=len(orders)),
...
async def participated_orders_paged(call: CallbackQuery):
    ...
    if not orders:
        ...
        return
    await _edit_panel(call,
        msg.participated_orders_list(page=page + 1, pages=max(1, (len(orders) + 4) // 5), total=len(orders)),
        reply_markup=paged_item_keyboard(
...
async def participated_order_detail(call: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        resource_status = '可领取' if can_claim else '还没收货，敬请期待哟 ⏳'
    ...
    await _edit_panel(call, text, reply_markup=participated_detail_keyboard(p.id if p else 0, can_claim=can_claim))
    await call.answer()
...
async def refund_orders_paged(call: CallbackQuery):
    ...
    if not refunds:
        ...
        return
    await _edit_panel(call,
        msg.refund_orders_list(page=page + 1, pages=max(1, (len(refunds) + 4) // 5), total=len(refunds)),
        reply_markup=paged_item_keyboard(
...
async def refresh_ticket_status(call: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if order.status == 'paid':
            ...
            # 根据车票类型动态展示普通车位、满员补票或发起人双车位专属卡片。
            await _edit_panel(
                call,
                _ticket_card_text(order, project),
                reply_markup=ticket_verify_keyboard(order.id),
            )
        else:
            await _edit_panel(
                call,
                msg.ticket_other_status(
                    ticket_no=_ticket_no(order.id),
                    project_no=project_no(project) if project else 'P.-',
                    blogger=project.blogger if project else '-',
                    status=order.status,
...
async def submit_order_prompt(call: CallbackQuery):
    # 兼容历史消息里的旧按钮；纯自动支付模式不再进入手动 VP 输入状态。
    await call.answer('当前使用自动核验，请点击「已付款，查询状态」或联系小掌柜。', show_alert=True)


# 纯自动支付绑定：购买成功通知只查 faka 一次，后续按钮使用已保存的核实付款记录。

async def _notify_verified_binding_failure(
    bot: Bot,
    *,
    verified: VerifiedPayment | None,
    reason: str,
    project: CrowdfundProject | None,
    order: PaymentOrder | None,
) -> None:
    system_no = verified.system_no if verified else '-'
    project_text = f'P.{int(project.id):03d}｜{project.blogger}' if project else '尚未选择项目'
    await bot.send_message(
        settings.ADMIN_GROUP_ID,
        '⚠️ 已核实付款本地绑定失败\n\n'
        f'用户：<code>{int(verified.user_id) if verified else "-"}</code>\n'
        f'系统单号：<code>{html.escape(system_no)}</code>\n'
        f'项目：{html.escape(project_text)}\n'
        f'车票：{_ticket_no(order.id) if order else "-"}\n'
        f'真实原因：{html.escape(reason)}',
        reply_markup=admin_payment_contact_keyboard(system_no),
        disable_web_page_preview=True,
    )


@router.callback_query(F.data.startswith('pay:verified_order:'))
async def bind_saved_verified_payment_to_order(call: CallbackQuery, bot: Bot):
    parts = (call.data or '').split(':')
    try:
        verified_id = int(parts[2])
        order_id = int(parts[3])
    except Exception:
        await call.answer('付款记录参数已失效，请联系小掌柜。', show_alert=True)
        return
    await call.answer('小掌柜正在绑座位～')
    async with SessionLocal() as session:
        outcome = await bind_verified_to_order(session, verified_id, order_id, call.from_user.id)
        if outcome.ok and outcome.order:
            await _edit_panel(
                call,
                payment_success_text(outcome.order, outcome.project),
                reply_markup=auto_payment_success_keyboard(),
            )
            await run_paid_followups(bot, session, outcome.order, notify_user=False)
            return
        await _edit_panel(
            call,
        ...
        await _notify_verified_binding_failure(
            bot, verified=outcome.verified, reason=outcome.reason, project=outcome.project, order=outcome.order
        )


@router.callback_query(F.data.startswith('pay:verified_project:'))
async def bind_saved_verified_payment_to_project(call: CallbackQuery, bot: Bot):
    parts = (call.data or '').split(':')
    try:
        verified_id = int(parts[2])
...
async def seed_secret_direct_verify(message: Message, bot: Bot):
    ...
    await confirm_seed_payment_message(
        message,
        bot,
...
async def hot_projects_callback(call: CallbackQuery):
    ...
    async with SessionLocal() as session:
        projects = await _load_hot_projects(session)
    if not projects:
        await _edit_panel(call, msg.hot_empty(), reply_markup=hot_empty_keyboard())
    else:
...
async def hot_project_view(call: CallbackQuery, bot: Bot):
    ...
    async with SessionLocal() as session:
        ...
        markup = hot_project_private_keyboard(project.id, full=_is_after_full_stage(project), cancelled=state_value(project.status) in ('cancelled', 'expired'), seat_price=project.seat_price)
...
async def creator_reimbursement_request(call: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        if w and w.status == 'paid':
            ...
            await _edit_panel(call, f'该项目报销已完成。\n{project_label(p)}\n金额：{w.creator_amount:g} 元', reply_markup=creator_project_detail_keyboard(p.id, status=state_value(p.status)))
            return
        if w and w.status == 'pending_admin':
            await call.answer('报销申请已提交，等待管理付款', show_alert=True)
            await _edit_panel(call, '你的报销资料已提交给管理，等待付款确认。', reply_markup=creator_project_detail_keyboard(p.id, status=state_value(p.status)))
            return
        if w is None:
    ...
    await state.update_data(withdrawal_id=w.id, project_id=p.id)
    ...
    await call.message.answer(
        ...
        reply_markup=payout_info_collect_keyboard(w.id, p.id),
    )
    await call.answer()
...
async def creator_withdraw_request(call: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        if active_request and active_request.status == 'pending_admin':
            await call.answer('已有一笔分润申请等待管理打款', show_alert=True)
            await _edit_panel(
                call,
            ...
            return
        if active_request and active_request.status == 'pending_info':
            w = active_request
            resumed = True
        else:
            pending_extra = max(0, int(p.extra_fund_count or 0) - int(p.extra_withdrawn_count or 0))
    ...
    await state.update_data(withdrawal_id=w.id, project_id=p.id)
    await state.set_state(ProfitWithdrawCollect.payout_info)
    title = '💰 继续填写提现收款资料' if resumed else '💰 已满足提现要求。'
    await call.message.answer(
        f'{title}\n\n'
        f'本次结算：满员后额外获取 {int(w.extra_count or 10)} 人一组\n'
        f'总额：{float(w.gross_amount or 0):g} 元\n'
        f'发起人 60%：{float(w.creator_amount or 0):g} 元\n'
        f'平台 40%：{float(w.platform_amount or 0):g} 元\n\n'
        f'请把收款方式发给我吧 (支付宝/TRX/USDT地址都行)\n'
        f'1）TRX/USDT 地址；或\n'
        f'2）支付宝收款信息/收款码；或\n'
        f'3）其他收款方式。\n\n'
    ...
    if resumed:
        await call.answer('请继续填写收款资料')
    else:
        await call.answer()


@router.callback_query(F.data.startswith('creator:payout_cancel:'))
async def creator_payout_cancel(call: CallbackQuery, state: FSMContext):
    parts = (call.data or '').split(':')
    try:
        withdrawal_id = int(parts[2])
        project_id = int(parts[3])
    except Exception:
        await call.answer('申请参数无效', show_alert=True)
        return
    async with SessionLocal() as session:
        ...
        if not p or p.creator_id != call.from_user.id:
            await call.answer('项目不存在或不是你发起的', show_alert=True)
            return
        if w and w.creator_id == call.from_user.id and w.status == 'pending_info':
...
async def collect_withdraw_info(message: Message, state: FSMContext, bot: Bot):
    ...
    await state.clear()
    await message.answer(user_text, reply_markup=creator_project_detail_keyboard(p.id, status=state_value(p.status)) if p else order_center_keyboard())
...
async def _after_admin_force_verify(bot: Bot, session, order: PaymentOrder, admin_id: int) -> None:
    ...
    if order.order_type == 'crowdfunding_creator_prepay':
        result_text = '车主双车位已核验并计入拼车进度。'
    elif order.order_type == 'crowdfunding_after_full':
        result_text = '满员后补票已核验，你已获得该资源的领取资格。'
    else:
        result_text = '补票已核验，你已正式上车。'
...
async def admin_order_detail(call: CallbackQuery, state: FSMContext):
    ...
    if order.project_id:
        ...
        rows.append([InlineKeyboardButton(text='✅ 已支付用户', callback_data=f'admin:paid_users:{order.project_id}')])
    ...
    text = (
        ...
        f'{msg.LINE}\n'
        '这里仅用于查看付款与绑定记录。待付、直接补单和同项目待付入口已隐藏，避免误触。'
    )
    await _edit_panel(call, text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    await call.answer()


@router.callback_query(F.data.startswith('admin:bind_pending_project:'))
async def admin_bind_pending_project(call: CallbackQuery, state: FSMContext):
    """只绑定用户已经存在的待付车票；没有待付车票时不再自动进入补单。"""
    if call.from_user.id not in settings.admin_id_list:
        await call.answer('无权限', show_alert=True)
        return
    ...
    for order in orders[:30]:
        ...
        rows.append([
            InlineKeyboardButton(
                text=f'绑定 {_ticket_no(order.id)}｜{user_label}｜{order.expected_amount:g}元'[:60],
                callback_data=f'admin:manual_verify_select:{order.id}',
            )
        ])
    rows.append([InlineKeyboardButton(text='➕ 没有待付车票，改补已付订单', callback_data=f'admin:add_paid_order_project:{project_id}')])
    rows.append([InlineKeyboardButton(text='⬅️ 返回项目详情', callback_data=f'admin:project:{project_id}')])

    if not orders:
        panel_text = (
            f'✅ 绑定待付车票\n\n{project_label(project)}\n\n'
            f'当前项目没有待付车票，所以不能走“绑定待付车票”。\n\n'
            f'如果用户确实已付款，但系统里没有待付车票，请点“补已付订单”。'
        )
    else:
        panel_text = (
            f'✅ 绑定待付车票\n\n{project_label(project)}\n\n'
            f'当前待付车票：{len(orders)} 张\n'
            f'请选择要绑定的车票，然后发送 VP 系统单号。'
        )
    await _edit_panel(call, panel_text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    await call.answer()


@router.callback_query(F.data.startswith('admin:add_paid_order_project:') | F.data.startswith('admin:manual_verify:'))
async def admin_add_paid_order_project(call: CallbackQuery, state: FSMContext):
    """用户没有待付车票但已经付款时，管理员手动补一张已支付车票。"""
    if call.from_user.id not in settings.admin_id_list:
        await call.answer('无权限', show_alert=True)
        return
    ...
    rows = [[InlineKeyboardButton(text='⛔ 取消补单，返回项目详情', callback_data=f'admin:manual_verify_cancel:{project_id}')]]
    panel_text = (
        f'➕ 补已付订单\n\n{project_label(project)}\n\n'
        f'适用场景：用户确实已付款，但系统里没有待付车票。\n\n'
...
def _parse_order_token(text: str | None) -> int | None:
    raw = (text or '').strip().upper().replace('T.', '').replace('T', '')
    try:
        value = int(raw)
    except Exception:
        return None
...
async def cancel_pending_order(call: CallbackQuery):
    ...
    await _edit_panel(
        ...
        reply_markup=cancel_pending_confirm_keyboard(order_id),
    )
    await call.answer()


@router.callback_query(F.data.startswith('orders:cancel_pending_confirm:'))
async def cancel_pending_order_confirm(call: CallbackQuery):
    order_id = int(call.data.split(':')[-1])
    async with SessionLocal() as session:
        o = await session.get(PaymentOrder, order_id)
        if not o or o.user_id != call.from_user.id or o.status != 'pending':
            ...
            return
        o.status = 'cancelled'
        o.fail_reason = '用户确认尚未付款并主动取消待支付订单'
        await session.commit()
    await _edit_panel(
        call,
        '✅ 小车票已经退好啦～\n━━━━━━━━━━━━━━\n\n要是想重新上车，重新进项目拉个新车车就好～\n\n━━━━━━━━━━━━━━\n小掌柜会帮你稳稳兜住，不会让你迷路 🎀',
        reply_markup=order_center_keyboard(),
    )
    await call.answer('已取消')
...
async def admin_dashboard_list(call: CallbackQuery):
    if not await _admin_group_allowed(call.from_user.id, call.message.chat.id):
        ...
        return
    callback_parts = (call.data or '').split(':')
    list_type = callback_parts[2] if len(callback_parts) >= 3 else ''
    ledger_day_offset = 0
    ledger_entry_page = 0
    if list_type == 'ledger':
        try:
            ledger_day_offset = max(0, int(callback_parts[3])) if len(callback_parts) >= 4 else 0
        except (TypeError, ValueError):
            ledger_day_offset = 0
        try:
            ledger_entry_page = max(0, int(callback_parts[4])) if len(callback_parts) >= 5 else 0
        except (TypeError, ValueError):
            ledger_entry_page = 0
    today_start = _beijing_today_start_utc_naive()
    async with SessionLocal() as session:
        if list_type == 'todos':
            ...
            day_start, day_end, day_label = _beijing_day_bounds_utc_naive(ledger_day_offset)
            entry_page_size = 20
            total_entries = int((await session.execute(
                select(func.count()).select_from(FinancialLedger).where(
                    FinancialLedger.created_at >= day_start,
                    FinancialLedger.created_at < day_end,
                )
            )).scalar() or 0)
            max_entry_page = max(0, (total_entries - 1) // entry_page_size)
            ledger_entry_page = min(ledger_entry_page, max_entry_page)
            rows_data = list((await session.execute(
                select(FinancialLedger)
                .where(FinancialLedger.created_at >= day_start, FinancialLedger.created_at < day_end)
                .order_by(FinancialLedger.created_at.desc(), FinancialLedger.id.desc())
                .offset(ledger_entry_page * entry_page_size)
                .limit(entry_page_size)
            )).scalars().all())
            totals = (await session.execute(
                select(
                    func.coalesce(func.sum(case((FinancialLedger.direction == 'income', FinancialLedger.amount), else_=0)), 0),
                    func.coalesce(func.sum(case((FinancialLedger.direction == 'expense', FinancialLedger.amount), else_=0)), 0),
                    func.coalesce(func.sum(case(
                        (FinancialLedger.direction == 'income', FinancialLedger.amount),
                        (FinancialLedger.direction == 'expense', -FinancialLedger.amount),
                        else_=0,
                    )), 0),
                ).where(FinancialLedger.created_at >= day_start, FinancialLedger.created_at < day_end)
            )).one()
            income_total, expense_total, balance = totals
            day_name = '今天' if ledger_day_offset == 0 else ('昨天' if ledger_day_offset == 1 else day_label)
            lines = [
                '💹 资金账本',
                '━━━━━━━━━━━━━━',
                f'日期：{day_label}（北京时间｜{day_name}）',
                f'收入：+{float(income_total or 0):g} 元',
                f'支出：-{float(expense_total or 0):g} 元',
                f'当日净额：{float(balance or 0):g} 元',
                f'流水：{total_entries} 条' + (f'｜本日第 {ledger_entry_page + 1}/{max_entry_page + 1} 页' if total_entries else ''),
                '━━━━━━━━━━━━━━',
            ]
            if not rows_data:
                lines.append('当天没有资金流水。')
            for item in rows_data:
                ...
                category_labels = {
                    'crowdfunding_join': '普通车位收入',
                    'crowdfunding_after_full': '满员补票收入',
                    'crowdfunding_creator_prepay': '发起人双车位收入',
                    'refund': '退款支出',
                    'reimbursement': '发起人报销',
                    'profit_withdrawal': '发起人分润提现',
                    'manual_transfer': '管理员转绑',
                    'manual_reassign_user': '管理员改归属',
                    'project_migration': '项目迁移',
                    'order_migration': '车票迁移',
                }
                category = category_labels.get(item.category or '', item.category or '-')
                description = (item.description or '').replace('\n', ' ').strip()
                description_line = f'｜说明：{description[:60]}' if description else ''
                lines.append(
                    f'\n{_fmt_dt(item.created_at)[11:]}｜{sign}{float(item.amount or 0):g} 元｜{category}'
                    f'\n项目：{project_display}｜博主：{blogger}｜用户：{item.user_id or "-"}{description_line}'
                )
            rows: list[list[InlineKeyboardButton]] = []
            entry_nav: list[InlineKeyboardButton] = []
            if ledger_entry_page > 0:
                entry_nav.append(InlineKeyboardButton(text='◀️ 本日前页', callback_data=f'admin:list:ledger:{ledger_day_offset}:{ledger_entry_page-1}'))
            if ledger_entry_page < max_entry_page:
                entry_nav.append(InlineKeyboardButton(text='本日后页 ▶️', callback_data=f'admin:list:ledger:{ledger_day_offset}:{ledger_entry_page+1}'))
            if entry_nav:
                rows.append(entry_nav)
            day_nav = [InlineKeyboardButton(text='⬅️ 前一天', callback_data=f'admin:list:ledger:{ledger_day_offset+1}:0')]
            if ledger_day_offset > 0:
                day_nav.append(InlineKeyboardButton(text='后一天 ➡️', callback_data=f'admin:list:ledger:{ledger_day_offset-1}:0'))
            rows.append(day_nav)
            if ledger_day_offset != 0:
                rows.append([InlineKeyboardButton(text='📅 回到今天', callback_data='admin:list:ledger:0:0')])
...
async def admin_support_reply_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        ...
        reply_markup=admin_support_reply_collect_keyboard(ticket_id),
    )
    await call.answer()


@router.callback_query(F.data.startswith('admin:support_reply_cancel:'))
async def admin_support_reply_cancel(call: CallbackQuery, state: FSMContext):
    if call.from_user.id not in settings.admin_id_list:
        await call.answer('无权限', show_alert=True)
        return
    ticket_id = int(call.data.split(':')[-1])
...
async def admin_contact_payment_user(call: CallbackQuery, bot: Bot):
    """Open/reopen a support dialog using the saved verified buyer first.

    Normal automatic payments do not query faka again. A fallback query is used
    only for failures that happened before a verified payment record could be saved.
    """
    support_admin_id = _support_private_admin_id()
    if not support_admin_id:
    ...
    if int(call.from_user.id) != int(support_admin_id):
        await call.answer('这个按钮只给客服管理员使用。', show_alert=True)
        return
    raw_no = (call.data or '').split(':')[-1]
    try:
        system_no = normalize_system_no(raw_no)
    except Exception:
        await call.answer('系统单号无效', show_alert=True)
        return

    await call.answer('正在锁定付款用户…')
    user_id = None
    username = None
    order_id = None
    project_id = None
    ...
    async with SessionLocal() as session:
        ...
        order = (await session.execute(
            select(PaymentOrder)
            .where(PaymentOrder.faka_system_no == system_no)
        ...
        if verified:
            ...
            detail = (
                f'系统单号：{system_no}\n'
                f'核实状态：{verified.status}\n'
                f'金额：{verified.amount} 元\n'
                f'商品：{verified.product_name}\n'
                f'商品类型：{verified.product_kind}'
            )
            project_id = int(verified.selected_project_id or 0) or None
            order_id = int(verified.bound_order_id or 0) or None
        if order:
            username = order.username
            order_id = int(order.id)
            project_id = int(order.project_id or 0) or project_id

    if user_id is None:
        # 仅用于“查单阶段就失败、尚未保存已核实付款记录”的管理员异常。
        try:
            result = await faka_query_client.query_order(system_no)
        except Exception as exc:
            await call.message.answer(f'❌ 仍然无法查询该订单，暂时不能确认用户。\n系统单号：{system_no}\n错误：{exc}')
            return
        if result.buyer_user_id is None or int(result.buyer_user_id or 0) <= 0:
            await call.message.answer(f'❌ faka 没有返回 Telegram 数字用户 ID，无法安全联系用户。\n系统单号：{system_no}')
            return
        user_id = int(result.buyer_user_id)
        detail = (
            f'系统单号：{system_no}\n'
            f'订单状态：{result.status or "-"}\n'
            f'金额：{result.amount if result.amount is not None else "-"} 元\n'
            f'商品：{result.product_name or "-"}'
        )

    try:
        await _open_admin_dialog_for_user(
            bot,
            ...
            project_id=project_id,
            order_id=order_id,
            ref_id=order_id or 0,
        )
    except Exception as exc:
        await call.message.answer(f'❌ 无法建立用户对话：{_friendly_support_delivery_error(exc)}')
        return
    await call.message.answer('✅ 已在客服管理员的机器人私聊中打开该用户对话。')


async def _admin_open_user_by_command(message: Message, state: FSMContext, bot: Bot, args: str | None, *, send_now: bool) -> None:
    if message.from_user.id not in settings.admin_id_list:
        await message.answer('无权限。')
        return
    if not _is_support_private_admin(message.from_user.id):
        await message.answer('该命令只给 SUPPORT_ADMIN_ID 对应管理员使用。')
        return
    user_id, body = _parse_admin_user_message_args(args)
    if not user_id:
        usage = '/msg 用户ID 要发送的内容' if send_now else '/contact 用户ID'
        await message.answer(f'用法：{usage}')
        return
    if send_now and not body:
        await message.answer('用法：/msg 用户ID 要发送的内容')
        return

    detail = f'管理员通过用户 ID 主动建立对话\n用户 ID：{user_id}'
    try:
        ticket, user_label = await _open_admin_dialog_for_user(
            bot,
            admin_id=message.from_user.id,
            user_id=user_id,
            source_page='direct',
            source_label='管理员主动联系',
            detail=detail,
            ref_id=user_id,
        )
    except Exception as exc:
        await message.answer(f'❌ 无法建立用户对话：{_friendly_support_delivery_error(exc)}')
        return
    ...
    if send_now:
        try:
            await send_message_direct(
                user_id,
                msg.support_user_reply(_support_no(ticket.id), body),
                reply_markup=contact_back_keyboard(),
            )
            async with SessionLocal() as session:
                db_ticket = await session.get(ContactTicket, ticket.id)
                if db_ticket:
                    db_ticket.status = 'answered'
                    db_ticket.admin_id = message.from_user.id
                    db_ticket.admin_reply = body
                    db_ticket.answered_at = datetime.utcnow()
                    db_ticket.last_error = None
                    await session.commit()
            await message.answer(f'✅ 已发送给用户 {user_label}，并已打开可继续回复的客服对话。')
        except Exception as exc:
            friendly = _friendly_support_delivery_error(exc)
            await _mark_support_delivery_failed(ticket.id, friendly)
            await message.answer(f'❌ 发送失败：{friendly}')
    else:
        await message.answer(f'✅ 已打开用户 {user_label} 的对话，请到机器人私聊回复新生成的对话卡片。')


@router.message(Command('contact', 'contact_user'))
async def admin_contact_user_command(message: Message, command: CommandObject, state: FSMContext, bot: Bot):
    """Open/reopen a support dialog directly by Telegram numeric user ID."""
    await _admin_open_user_by_command(message, state, bot, command.args, send_now=False)


@router.message(Command('msg', 'send_user'))
async def admin_send_user_command(message: Message, command: CommandObject, state: FSMContext, bot: Bot):
    """Send a text message directly by Telegram numeric user ID and keep the dialog open."""
    await _admin_open_user_by_command(message, state, bot, command.args, send_now=True)
...
async def admin_support_private_reply_to_user(message: Message, state: FSMContext, bot: Bot):
    ...
    if command_text in {'/contact', '/contact_user', '/msg', '/send_user'} or command_text.startswith(('/contact ', '/contact_user ', '/msg ', '/send_user ')):
        await state.clear()
        command_name, _, args = command_text.partition(' ')
        await _admin_open_user_by_command(
            message,
            state,
            bot,
            args,
            send_now=command_name in ('/msg', '/send_user'),
        )
        return
    ...
    if command_text.startswith('/search'):
        ...
        if len(parts) == 2:
            ...
            await message.answer(msg.admin_search_help(), reply_markup=admin_search_prompt_keyboard())
        return
...
async def refund_apply_start(call: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        if r.status == 'refunded':
            ...
            await call.message.answer(msg.refund_already_done(refund_no=_refund_no(r.id), amount=float(r.amount or 0)), reply_markup=refund_detail_keyboard(r.id))
            return
        if r.status == 'pending_admin':
            await call.answer('你已提交退款资料，请等待管理处理', show_alert=True)
            await call.message.answer(msg.refund_already_submitted(_refund_no(r.id)), reply_markup=refund_detail_keyboard(r.id))
            return
...
async def collect_refund_info(message: Message, state: FSMContext, bot: Bot):
    ...
    await state.clear()
    await message.answer(msg.refund_user_submitted(_refund_no(refund_id)), reply_markup=refund_detail_keyboard(refund_id))
...
async def admin_refund_done(call: CallbackQuery, bot: Bot):
    ...
    async with SessionLocal() as session:
        ...
        try:
            await bot.send_message(
                ...
                reply_markup=refund_detail_keyboard(r.id),
            )
        except Exception as exc:
            notify_error = exc
        user_id = r.user_id
    ...
    admin_markup = admin_project_detail_keyboard(project_id) if project_id else admin_dashboard_keyboard()
    await call.message.answer(msg.refund_done_admin(refund_no=_refund_no(refund_id), user_id=user_id, amount=amount, notify_error=notify_error), reply_markup=admin_markup)
    await call.answer()
...
ADMIN_TOOL_HELP = {
    ...
    'reply': (
        '💬 回复客户\n\n'
        '按工单回复：/reply S.工单号 回复内容\n'
        '按用户 ID 打开对话：/contact 用户ID\n'
        '按用户 ID 直接发送：/msg 用户ID 要发送的内容\n\n'
        '示例：\n'
        '/reply S.012 好的，我帮你看一下～\n'
        '/contact 1566554813\n'
        '/msg 1566554813 你的付款已经收到，我正在帮你处理。\n\n'
        '用户即使点过“结束对话”，/contact 和 /msg 也会重新建立客服会话。'
    ),
}
...
@router.callback_query(F.data == 'admin:search_cancel')
async def admin_search_cancel(call: CallbackQuery, state: FSMContext):
    if not await _admin_group_allowed(call.from_user.id, call.message.chat.id):
        await call.answer('只能在审核群或管理员私聊使用', show_alert=True)
        return
    await state.clear()
    await _edit_panel(call, '⛔ 已取消本次搜索。', reply_markup=admin_dashboard_keyboard())
    await call.answer('已取消搜索')
