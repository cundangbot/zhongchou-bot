...
class CrowdfundProject(Base):
    ...
    discussion_root_message_id: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
    discussion_detail_message_id: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
...
class VerifiedPayment(Base):
    __tablename__ = 'verified_payments'

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    system_no: Mapped[str] = mapped_column(String(96), unique=True, index=True)
    pay_no: Mapped[str | None] = mapped_column(String(128), nullable=True)
    user_id: Mapped[int] = mapped_column(BigInteger, index=True)
    buyer_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    amount: Mapped[Decimal] = mapped_column(MONEY)
    product_name: Mapped[str] = mapped_column(String(255))
    product_kind: Mapped[str] = mapped_column(String(32), index=True)
    pay_channel: Mapped[str | None] = mapped_column(String(64), nullable=True)
    pay_method: Mapped[str | None] = mapped_column(String(64), nullable=True)
    order_bot: Mapped[str | None] = mapped_column(String(128), nullable=True)
    raw_response: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(String(32), default='verified_unbound', index=True)
    selected_project_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    bound_order_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    failure_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    user_notice_sent_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    user_notice_error: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, index=True)
