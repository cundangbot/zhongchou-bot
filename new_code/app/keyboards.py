...
def pending_order_actions_keyboard(order_id: int, payment_link: str | None = None) -> InlineKeyboardMarkup:
    """Pure automatic payment flow: pay, query status, or cancel before paying."""
    rows = []
    if payment_link:
        rows.append([InlineKeyboardButton(text='💳 立即支付', url=payment_link)])
    rows.append([InlineKeyboardButton(text='🔄 已付款，查询状态', callback_data=f'pay:refresh:{order_id}')])
    rows.append([InlineKeyboardButton(text='🗑 未付款，取消车票', callback_data=f'orders:cancel_pending:{order_id}')])
    rows.append([InlineKeyboardButton(text='📋 返回我的众筹', callback_data='orders:center')])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def auto_payment_success_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='🎟️ 查看已上车票', callback_data='orders:participated:0')],
        [InlineKeyboardButton(text='📋 返回我的众筹', callback_data='orders:center')],
    ])


def auto_payment_choice_keyboard(choices: list[tuple[int, str]], verified_id: int) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text=label[:60], callback_data=f'pay:verified_order:{int(verified_id)}:{order_id}')]
        for order_id, label in choices[:20]
    ]
    rows.append([support_contact_button('💬 联系小掌柜', 'payment', 0)])
    rows.append([InlineKeyboardButton(text='📋 返回我的众筹', callback_data='orders:center')])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def auto_payment_project_keyboard(choices: list[tuple[int, str]], verified_id: int) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text=label[:60], callback_data=f'pay:verified_project:{int(verified_id)}:{project_id}')]
...
def cancel_pending_confirm_keyboard(order_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='✅ 未付款，取消车票', callback_data=f'orders:cancel_pending_confirm:{order_id}')],
        [InlineKeyboardButton(text='🔙 不取消，返回车票', callback_data=f'orders:pending_detail:{order_id}')],
    ])
...
def hot_empty_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='🚗 发起众筹', callback_data='cf:start')],
        [InlineKeyboardButton(text='📋 返回我的众筹', callback_data='orders:center')],
    ])


def hot_project_private_keyboard(project_id: int, *, full: bool = False, cancelled: bool = False, seat_price: float | int | None = None) -> InlineKeyboardMarkup:
    rows = list(join_project_keyboard(project_id, full=full, cancelled=cancelled, seat_price=seat_price).inline_keyboard)
    rows.append([InlineKeyboardButton(text='⬅️ 返回热门众筹', callback_data='hot:list:0')])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def creator_resource_keyboard(project_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text='🚗 查看车主项目', callback_data=f'orders:created_detail:{project_id}')],
    ])
...
def admin_project_detail_keyboard(project_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='🧩 生成拼车模板', callback_data=f'admin:regenerate_panel:{project_id}')],
        [InlineKeyboardButton(text='🗑 彻底删除项目', callback_data=f'admin:delete_project:{project_id}')],
        [InlineKeyboardButton(text='🔍 刷新项目详情', callback_data=f'admin:project:{project_id}')],
...
def resource_review_keyboard(project_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='📦 查看/修改资源', callback_data=f'admin:view_resources:{project_id}')],
        ...
        [InlineKeyboardButton(text='🔎 返回项目详情', callback_data=f'admin:project:{project_id}')],
        [InlineKeyboardButton(text='⬅️ 返回管理面板', callback_data='admin:dashboard')],
    ])
...
def crowdfunding_cancel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='⛔ 不发了', callback_data='cf:cancel')],
    ])


def creator_buyinfo_collect_keyboard(project_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='⛔ 暂不填写，返回车主项目', callback_data=f'creator:buyinfo_cancel:{project_id}')],
    ])


def payout_info_collect_keyboard(withdrawal_id: int, project_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='⛔ 暂不提交，返回车主项目', callback_data=f'creator:payout_cancel:{withdrawal_id}:{project_id}')],
    ])
...
def resource_next_page_keyboard(project_id: int, kind: str, next_page: int | None) -> InlineKeyboardMarkup:
    rows = []
    if next_page is not None:
        rows.append([InlineKeyboardButton(text='📖 下一页', callback_data=f'resource:page:{project_id}:{kind}:{next_page}')])
    rows.append([InlineKeyboardButton(text='🔙 返回资源详情', callback_data=f'resources:detail:{project_id}')])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def admin_dashboard_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='🧺 今日待处理', callback_data='admin:list:todos')],
...
def ticket_verify_keyboard(order_id: int, back_to_pending: bool = True) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text='🔄 刷新核验结果', callback_data=f'pay:refresh:{order_id}')],
        [support_contact_button('💬 联系小掌柜', 'payment', order_id)],
    ]
    if back_to_pending:
        rows.append([InlineKeyboardButton(text='⬅️ 返回待付车票', callback_data='orders:pending:0')])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def participated_detail_keyboard(project_id: int, can_claim: bool = False) -> InlineKeyboardMarkup:
    rows = []
    if can_claim:
...
def creator_project_detail_keyboard(project_id: int, *, status: str | None = None, can_relaunch: bool = False) -> InlineKeyboardMarkup:
    ...
    if value in {'active', 'approved_wait_creator', 'full', 'waiting_creator_resource', 'waiting_buy_info', 'platform_purchasing', 'admin_uploading', 'resource_submitted', 'resource_review', 'resource_rejected'}:
...
def admin_support_reply_collect_keyboard(ticket_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='⛔ 取消回复，返回客服工单', callback_data=f'admin:support_reply_cancel:{ticket_id}')],
    ])
...
def admin_search_prompt_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='⛔ 取消搜索，返回管理面板', callback_data='admin:search_cancel')],
    ])
...
def admin_search_results_keyboard(projects=None, orders=None, refunds=None, tickets=None, payouts=None) -> InlineKeyboardMarkup:
    ...
    for o in list(orders or [])[:5]:
        ...
        if pid:
            rows.append([InlineKeyboardButton(text='🔍 对应项目', callback_data=f'admin:project:{pid}')])
...
def admin_tools_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='📋 项目列表', callback_data='admin:projects:list:0'), InlineKeyboardButton(text='🔁 迁移旧车用户', callback_data='admin:tool:migrate')],
        [InlineKeyboardButton(text='➕ 补已付订单', callback_data='admin:tool:add_order'), InlineKeyboardButton(text='👤 车票改用户', callback_data='admin:tool:rebind_user')],
        [InlineKeyboardButton(text='↔️ 转移付款车票', callback_data='admin:tool:move_bind'), InlineKeyboardButton(text='🎫 迁移单张车票', callback_data='admin:tool:migrate_order')],
        [InlineKeyboardButton(text='💬 回复客户', callback_data='admin:tool:reply')],
        [InlineKeyboardButton(text='⬅️ 返回管理面板', callback_data='admin:dashboard')],
    ])
