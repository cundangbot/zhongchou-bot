...
from app.db.models import PaymentOrder, CrowdfundProject, ResourceAccess, RiskLog, UserBlacklist, RefundRecord
...
def verify_faka_result(local: PaymentOrder, result: FakaOrderResult, current_user_id: int) -> tuple[bool, str]:
    ...
    product_keywords = settings.payment_expected_product_keywords
    if product_keywords:
        product_name = (result.product_name or '').strip()
        if not product_name:
            return False, '发卡机器人未返回商品名称'
        if not any(keyword in product_name for keyword in product_keywords):
            return False, f'商品名称不匹配：{product_name}'
    return True, '支付确认成功'


async def confirm_order_by_system_no(
    session: AsyncSession,
    user_id: int,
    system_no: str,
    order_id: int | None = None,
    *,
    prefetched_result: FakaOrderResult | None = None,
    ...
    if order_id is not None:
        ...
        if local.status == 'paid':
            return True, '这张车票已经核验成功，无需重复处理', local
    ...
    if is_seed:
        ...
        result = prefetched_result or await faka_query_client.query_order(system_no)
    ...
    await post_ledger(
        ...
        user_id=local.user_id, description=f'车票 {_no(local.id)} 核验成功',
    )
        ...
        if project:
            ...
            if order.order_type == 'crowdfunding_creator_prepay':
                ...
                if state_value(project.status) == ProjectState.APPROVED_WAIT_CREATOR.value:
                    await transition_project(session, project, ProjectState.ACTIVE, reason='发起人预付核验成功', idempotency_key=f'project:{project.id}:creator-paid')
