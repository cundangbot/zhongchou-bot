...
底部常驻菜单新增「💎 会员购买」。用户点击后会看到当前已发布及未来完成的众筹项目资源权益说明，并可通过 `MEMBER_GROUP_PAYMENT_LINK` 跳转独立支付页。该支付入口与四种车位商品隔离，不会触发 VP 自动绑票。


## v1.6.4.10 菜单、异常计数与历史面板修复

- 主菜单固定四个业务按钮。
- 管理面板的系统异常改为按异常类型计数；同类重复事件折叠更新，不再显示几十条重复异常。
- 新项目首次发布即使用“等待发起人双车位”状态；白名单自动核验后仅更新当前新项目一次，用于显示已锁定两个座位和正式开放上车。


## v1.6.4.11 会员菜单与启动行为统一

- 原会员入口统一命名为「💎 会员购买」，并放在主菜单第三个位置。
- 主菜单顺序固定为：发起众筹、热门众筹、会员购买、我的众筹。
- 删除 `/menu`、`/version` 命令及对应处理器，不再依赖强制刷新入口。
- 部署和重启时永久不扫描历史待预付项目、不自动处理旧项目、不修改历史频道拼车模板。
- 发起人白名单仅在新项目审核通过当下自动核验，并只更新当前新项目的模板。


## v1.6.4.12 稳定性与业务上下文小优化

- 启动业务任务前检查 Alembic 版本和支付关键表；不一致时仅发送一次简短管理员告警，并暂停支付监听与调度器。
- 系统健康页增加支付核验红绿灯、等待本地处理、等待用户选择、人工关注、最后 faka 查单和最近付款异常。
- 异常付款通知增加 faka 结果、匹配待付票、选择项目、重试本地绑定和联系付款用户五个按钮。
- 同一项目几秒内连续变化时合并为一次频道面板更新，默认 `PROJECT_PANEL_DEBOUNCE_SECONDS=3`。
- 退款详情补齐原因、原支付、支付单号、支付方式和时间；仅项目取消、项目过期或资源上传超时可申请退款。
- 每段客服会话仅第一条自动携带来源、项目/车票/VP 状态、最后错误和最近三个业务操作；结束对话后新会话重新携带一次。
- 其他菜单、会员购买、自动支付商品、白名单双车位、资源交付、账本和历史面板规则保持不变。
