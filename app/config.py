...
class Settings(BaseSettings):
    ...
    # 自动验票：监听“购买成功”通知机器人发来的消息，提取 VP 单号后自动向 FAKA_BOT_USERNAME 查单。
    # 若通知机器人就是下单机器人，可保持与 EXPECTED_FAKA_ORDER_BOT 一致。
    PAYMENT_AUTO_CONFIRM_ENABLED: bool = True
    PAYMENT_CONFIRM_BOT_USERNAME: str = ''
    # 管理员手动补单的底层商品关键词校验。自动购买监听使用 payment_products.py 的四种精确商品映射。
    PAYMENT_EXPECTED_PRODUCT_KEYWORDS: str = ''
    ...
    PROJECT_PANEL_DEBOUNCE_SECONDS: float = 3.0
    ...
    def payment_expected_product_keywords(self) -> List[str]:
        return [x.strip() for x in self.PAYMENT_EXPECTED_PRODUCT_KEYWORDS.split(',') if x.strip()]
