...
from app.services.payment_auto_listener import payment_auto_listener, resume_unbound_verified_payments
...
def setup_scheduler(bot: Bot) -> AsyncIOScheduler:
    ...
    async def check_pending_orders_job():
        async with SessionLocal() as session:
            ...
            for order in reminders:
                ...
                action_hint = '请点击「💳 立即支付」。付款成功后系统会自动核验并通知你，无需额外操作。'
                sent = await _safe_send(
                    bot,
                    order.user_id,
                    f'⏳ 待付车票即将过期，请尽快完成付款～\n\n'
        ...
        async with SessionLocal() as session:
            ...
            for p in cancelled:
                ...
                await _safe_send(
                    ...
                    p.creator_id,
                    f'⛔ 拼车失败\n\n{project_label(p)}\n\n'
                    f'原因：你在 {settings.PENDING_ORDER_EXPIRE_MINUTES} 分钟内未完成发起人双车位支付，系统已取消本次拼车。'
                )
                await _safe_send(
                    bot,
    ...
    async def check_telethon_health_job():
        if settings.PAYMENT_MODE == 'telethon' and not settings.PAYMENT_TEST_MODE:
            connected = await faka_query_client.ensure_connected(notify=True)
            if connected and settings.PAYMENT_AUTO_CONFIRM_ENABLED:
                if not payment_auto_listener.is_running:
                    await payment_auto_listener.start(bot)
                await resume_unbound_verified_payments(bot, limit=50)
